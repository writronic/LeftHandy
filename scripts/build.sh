...
KEYCHAIN_PROFILE="${KEYCHAIN_PROFILE:-LeftHandy}"

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
XCODEPROJ="${PROJECT_ROOT}/LeftHandy.xcodeproj"

ENTITLEMENTS_MAIN="${PROJECT_ROOT}/LeftHandy/LeftHandy.entitlements"
...
if [[ "$ARCH" == "universal" ]]; then
    DMG_NAME="LeftHandy.dmg"
else
    DMG_NAME="LeftHandy-${ARCH}.dmg"
fi

DMG_OUTPUT="${OUTPUT_DIR}/${DMG_NAME}"

echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  LeftHandy Build & DMG — ${ARCH}                           "
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""
...
if [[ "$ARCH" == "universal" ]]; then
    ...
    lipo -info "$APP_PATH/Contents/MacOS/LeftHandy"
else
...
# Main app (sign directly — no embedded frameworks to sign separately)
echo "  Signing LeftHandy.app..."
...
echo "✅ Phase 2 complete: App re-signed."
echo ""
